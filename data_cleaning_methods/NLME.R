...
#set up starting values based on NLR model and run the NLME model  
  ...
  intervals <- seq(min(dat$age), max(dat$age+365), 365)
  bin_freq <- get_intervals(dat, intervals, 100)
...
#add prediction intervals to original data
  ...
  dat <- dat %>%
    ...
    mutate(random_effects = predict(mixd, newdata = master_DNC, level = 1),
           ...
           upper_rand = both_effects + (4*smoothed_sd_residuals),
           lower_rand = both_effects - (4*smoothed_sd_residuals),
           upper_fixd = both_effects + (4*total_variation),
           lower_fixd = both_effects - (4*total_variation),
           upper = case_when(!is.na(random_effects) & num_data_entries > 1 ~ upper_rand,
                             !is.na(random_effects) & num_data_entries < 2 ~ upper_fixd,
                             is.na(random_effects) ~ upper_fixd),
           lower = case_when(!is.na(random_effects) & num_data_entries > 1 ~ lower_rand,
                             !is.na(random_effects) & num_data_entries < 2 ~ lower_fixd,
                             is.na(random_effects) ~ lower_fixd))
  
  
  get_outliers <- function(X, var1 = "new_weight", upper_limit = "upper", 
                           lower_limit = "lower", outlier_name = "outlier", print_results = FALSE) {
