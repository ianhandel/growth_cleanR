...
#Ensure working directory is set to the main folder  
  setwd("/Users/s1576473/dev/growth_cleanR")
...
#and specificity and monitors progress
  ...
  get_se_and_sp <- function(master_data, data_name, converge_fail = FALSE) {
    ...
    get_progress()
    replay(evaluate(file('/Users/s1576473/dev/growth_cleanR/data_cleaning_methods/SZCO.R')))
    get_progress()
    replay(evaluate(file('/Users/s1576473/dev/growth_cleanR/data_cleaning_methods/LZCO.R')))
    get_progress()
    ...
    #If the NLME model did not fail to converge, run methods    
      if(exists('mixd') == TRUE) {
        ...
        replay(evaluate(file('/Users/s1576473/dev/growth_cleanR/data_cleaning_methods/NLME_A.R')))
        get_progress()
      }
    }
...
#This is the model that is used by the NLR and NLME methods - it needs defining 
