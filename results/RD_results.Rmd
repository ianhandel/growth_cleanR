...
*Graph of data before RD method*
...
```{r}
#DELETIONS
  deletions <- subset(dat, !(dat$row_ID %in% master_RD$row_ID)) 

#percentage of data deleted
  del <- 100 - ((length(master_RD$ind_ID)/length(dat$ind_ID))*100)
...
#impluasible values deletions 
  deletions_5 <- subset(dat4, !(dat4$row_ID %in% master_RD$row_ID))
}
...
#MODIFICATIONS
  ...
  modifications <- subset(master_RD,master_RD$changed_weight == TRUE) 
...
*Overall Summary of RD Method Performance (after data cleaning)*


Mean = `r mean(master_RD$new_weight)`

Median = `r median(master_RD$new_weight)`

SD = `r sd(master_RD$new_weight)`

Range = `r range(master_RD$new_weight)`

Number of observations = `r length(factor(master_RD$ind_ID))`

Number of individuals =  `r length(unique(factor(master_RD$ind_ID)))`
...
*Graph of data after RD method*

```{r}
ggplot(master_RD, aes(x = age/365, y = new_weight, group = ind_ID, colour = sex, fill= sex)) +
                geom_line(alpha = 0.4, lwd = 0.2) +
                geom_point(alpha = 0.4) +
