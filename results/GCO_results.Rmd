...
```{r}
#DELETIONS
  deletions <- subset(dat, !(dat$row_ID %in% master_GCO$row_ID)) 

#percentage of data deleted
  del <- 100 - ((length(master_GCO$ind_ID)/length(dat$ind_ID))*100)
...
#impluasible values deletions 
  deletions_5 <- subset(dat4, !(dat4$row_ID %in% master_GCO$row_ID))
}
...
#MODIFICATIONS
  ...
  modifications <- subset(master_GCO,master_GCO$changed_weight == TRUE) 
...
Mean = `r mean(master_GCO$new_weight)`

Median = `r median(master_GCO$new_weight)`

SD = `r sd(master_GCO$new_weight)`

Range = `r range(master_GCO$new_weight)`

Number of observations = `r length(factor(master_GCO$ind_ID))`

Number of individuals =  `r length(unique(factor(master_GCO$ind_ID)))`
...
```{r}
ggplot(master_GCO, aes(x = age/365, y = new_weight, group = ind_ID, colour = sex, fill= sex)) +
                geom_line(alpha = 0.4, lwd = 0.2) +
                geom_point(alpha = 0.4) +
