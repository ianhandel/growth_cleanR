...
*Graph of data before NLME_A method*
...
```{r}
#DELETIONS
  deletions <- subset(dat, !(dat$row_ID %in% master_NLME_A$row_ID)) 

#percentage of data deleted
  del <- 100 - ((length(master_NLME_A$ind_ID)/length(dat$ind_ID))*100)

#ALGORITHM DELETIONS (ONLY RELEVANT TO NLME-A AND NLME-A METHODS!)
if(exists("dat3") == TRUE) {
#jumping deletions 
...
#impluasible values deletions 
  deletions_5 <- subset(dat4, !(dat4$row_ID %in% master_NLME_A$row_ID))
}
...
#MODIFICATIONS
  master_NLME_A <- master_NLME_A %>% 
    mutate(changed_weight = weight != new_weight)
  
  modifications <- subset(master_NLME_A,master_NLME_A$changed_weight == TRUE) 
...
*Overall Summary of NLME_A Method Performance (after data cleaning)*


Mean = `r mean(master_NLME_A$new_weight)`

Median = `r median(master_NLME_A$new_weight)`

SD = `r sd(master_NLME_A$new_weight)`

Range = `r range(master_NLME_A$new_weight)`

Number of observations = `r length(factor(master_NLME_A$ind_ID))`

Number of individuals =  `r length(unique(factor(master_NLME_A$ind_ID)))`
...
*Graph of data after NLME_A method*

```{r}
ggplot(master_NLME_A, aes(x = age/365, y = new_weight, group = ind_ID, colour = sex, fill= sex)) +
                geom_line(alpha = 0.4, lwd = 0.2) +
                geom_point(alpha = 0.4) +
