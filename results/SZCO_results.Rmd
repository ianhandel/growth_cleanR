...
*Graph of data before SZCO method*
...
```{r}
#DELETIONS
  deletions <- subset(dat, !(dat$row_ID %in% master_SZCO$row_ID)) 

#percentage of data deleted
  del <- 100 - ((length(master_SZCO$ind_ID)/length(dat$ind_ID))*100)
...
#impluasible values deletions 
  deletions_5 <- subset(dat4, !(dat4$row_ID %in% master_SZCO$row_ID))
}
...
#MODIFICATIONS
  master_SZCO <- master_SZCO %>% 
    mutate(changed_weight = weight != new_weight)
  
  modifications <- subset(master_SZCO,master_SZCO$changed_weight == TRUE) 
...
*Overall Summary of SZCO Method Performance (after data cleaning)*


Mean = `r mean(master_SZCO$new_weight)`

Median = `r median(master_SZCO$new_weight)`

SD = `r sd(master_SZCO$new_weight)`

Range = `r range(master_SZCO$new_weight)`

Number of observations = `r length(factor(master_SZCO$ind_ID))`

Number of individuals =  `r length(unique(factor(master_SZCO$ind_ID)))`
...
*Graph of data after SZCO method*

```{r}
ggplot(master_SZCO, aes(x = age/365, y = new_weight, group = ind_ID, colour = sex, fill= sex)) +
                geom_line(alpha = 0.4, lwd = 0.2) +
                geom_point(alpha = 0.4) +
